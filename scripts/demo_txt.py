...
tf.app.flags.DEFINE_string(
        'pred_dir', '../SemSeg_LIDAR/results/squeeze/test1/', """SemSeg predictions.""")
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
      ...
      for f in glob.iglob(FLAGS.input_path):
        ...
        # Map prediction 
        pred_aux = pred_cls.swapaxes(0, 1)
        pred_aux = pred_aux.swapaxes(1, 2)
        prediciton = np.concatenate((lidar_aux, pred_aux), axis=2)
        print(prediciton.shape)
        np.save(os.path.join(FLAGS.pred_dir, 'pred_' + file_name + '.npy'), 
        	prediciton)
